...
import type { SolverWorkerInputV2, SolverWorkerOutputV2, SolverWorkerErrorV2 } from '../types';

export function useSolverWorkerV2() {
  const workerRef = useRef<Worker | null>(null);

  const runSolver = useCallback((input: SolverWorkerInputV2): Promise<SolverWorkerOutputV2> => {
    return new Promise((resolve, reject) => {
      // Create worker if it doesn't exist
      if (!workerRef.current) {
        console.log('[useSolverWorkerV2] Creating new worker...');
        workerRef.current = new Worker(new URL('../workers/solverWorkerV2.ts', import.meta.url), { type: 'module' });
      }

      const worker = workerRef.current;

      // Set up message handler
      const handleMessage = (e: MessageEvent<SolverWorkerOutputV2 | SolverWorkerErrorV2>) => {
